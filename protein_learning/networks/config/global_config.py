...
from scoring.loss.coord_loss import COORD_LOSS_TY_DESCS, CoordLossTys
...
def get_config(arg_list):
    ...
    parser = ArgumentParser(
        description="Global Configuration Settings",  # noqa
    ...
    model_settings.add_argument("--sc_atom_mode", type=str, default="all")
    ...
    model_settings.add_argument(
        "-d", "--depth", help="transformer depth", type=int, default=3
    )
    ...
    model_settings.add_argument("--pair_bias", action="store_true", dest="pair_bias")
    model_settings.add_argument(
        "--init_eps",
        type=float,
        default=None,
    ...
    atom_feat_settings.add_argument(
        "--atom_dim_in",
        type=int,
        default=128,
    ...
    coord_feat_settings.add_argument(
        "-n",
        "--coord_heads",
    ...
    coord_feat_settings.add_argument(
        "-i",
        "--coord_dim_hidden",
        help="hidden dimension to use for coord features" "defaults to atom_dim_hidden",
        type=int,
        default=None,
    )
    coord_feat_settings.add_argument(
        "--coord_dim_out",
        help=" dimension to use for coord output features" "defaults to 4",
        type=int,
        default=4,
    )
    ...
    coord_feat_settings.add_argument(
        "--use_rel_coord_input",
        action="store_true",
        help="whether to use CA-X relative position as" " input to se3 transformer",
    )
    ...
    edge_feat_settings.add_argument(
        "--no_use_edge_feats", action="store_false", dest="use_edge_feats"
    )
    ...
    global_attn_settings.add_argument(
        "--use_global_attn",
        action="store_true",
    ...
    feature_gen.add_argument(
        "--one_hot_features", action="store_true", dest="one_hot_features"
    )
    ...
    feature_gen.add_argument(
        "--no_pre_embed_rosetta_score_terms",
        action="store_false",
    ...
    res_embedding_opts.add_argument(
        "--embed_rel_solv_acc", action="store_true", help=""
    )
    res_embedding_opts.add_argument(
        "--rel_solv_acc_embed_bins", type=int, default=10, help=""
    )
    res_embedding_opts.add_argument(
        "--rel_solv_acc_embed_dim", type=int, default=8, help=""
    )
    ...
    res_embedding_opts.add_argument(
        "--dihedral_embed_bins", type=int, default=48, help=""
    )
    res_embedding_opts.add_argument(
        "--dihedral_embed_dim", type=int, default=6, help=""
    )

    res_embedding_opts.set_defaults(fourier_encode_dihedral=False)
    res_embedding_opts.add_argument(
        "--fourier_encode_dihedral",
        action="store_true",
        dest="fourier_encode_dihedral",
        help=" fourier encode phi and psi dihedral angles",
    )
    res_embedding_opts.add_argument(
        "--dihedral_fourier_feats", type=int, default=1, help=""
    )
    ...
    res_pair_opts = parser.add_argument_group(
        "Residue Pair Embedding + Encoding Options"
    )

    res_pair_opts.set_defaults(embed_res_pairs=False)
    res_pair_opts.add_argument(
        "--embed_res_pairs",
        action="store_true",
    ...
    res_pair_opts.add_argument(
        "--joint_embed_res_pair_n_rel_sep",
        action="store_true",
    ...
    res_pair_opts.add_argument(
        "--fourier_encode_rel_sep",
        action="store_true",
        dest="fourier_encode_rel_sep",
        help="whether to include fourier encodings for relative seq. separation",
    )
    res_pair_opts.add_argument(
        "--num_fourier_rel_sep_feats", type=int, default=4, help=""
    )
    ...
    res_pair_opts.add_argument(
        "--rel_sep_embed_dim", type=int, default=8, help="relative separation embed dim"
    )
    res_pair_opts.set_defaults(use_small_sep_bins=False)
    res_pair_opts.add_argument(
        "--use_small_sep_bins", action="store_true", dest="use_small_sep_bins"
    )

    # Tr-Rosetta orientation features
    res_pair_opts.set_defaults(use_tr_rosetta_ori_features=True)
    res_pair_opts.add_argument(
        "--no_use_tr_rosetta_ori_features",
    ...
    experimental_settings.add_argument(
        "--use_dist_conv", action="store_true", default=False
    )
    experimental_settings.add_argument(
        "--pairwise_dist_conv", action="store_true", default=False
    )
    ...
    experimental_settings.add_argument(
        "--pre_transformer_node_dropout", default=0.15, type=float
    )
    experimental_settings.add_argument(
        "--pre_transformer_edge_dropout", default=0.25, type=float
    )

    experimental_settings.add_argument(
        "--pre_transformer_gpu_index",
        type=int,
        default=None,
    ...
    experimental_settings.add_argument(
        "--pre_transformer_edge_consensus", action="store_true"
    )

    experimental_settings.set_defaults(use_nbr_attn=False)
    experimental_settings.add_argument("--use_nbr_attn", action="store_true")

    experimental_settings.set_defaults(mask_unsampled_edges=False)
    experimental_settings.add_argument("--mask_unsampled_edges", action="store_true")

    experimental_settings.set_defaults(symmetrize_edges=False)
    experimental_settings.add_argument("--symmetrize_edges", action="store_true")
    experimental_settings.add_argument(
        "--pre_transformer_max_radius", default=24, type=float
    )
    experimental_settings.add_argument(
        "--pre_transformer_max_nbrs", default=50, type=int
    )
    experimental_settings.add_argument(
        "--pre_transformer_edge_attn_heads", default=4, type=int
    )
    experimental_settings.add_argument(
        "--pre_transformer_edge_attn_dim", default=32, type=int
    )
    experimental_settings.set_defaults(use_pre_transformer_update_bias=False)
    experimental_settings.add_argument(
        "--use_pre_transformer_update_bias", action="store_true"
    )

    experimental_settings.add_argument(
        "--pre_transformer_heads", type=int, default=8, help=""
    )

    experimental_settings.add_argument(
        "--pre_transformer_dim_head", type=int, default=32, help="head dimension"
    )

    experimental_settings.add_argument(
        "--pre_transformer_dim_hidden", type=int, default=256, help="hidden dimension"
    )

    experimental_settings.set_defaults(pre_transformer_edge_attn=True)
    experimental_settings.add_argument(
        "--no_use_pre_transformer_edge_attn",
        action="store_false",
    ...
    experimental_settings.add_argument(
        "--random_sample_tri_attn", action="store_true", dest="random_sample_tri_attn"
    )
    experimental_settings.set_defaults(reuse_tri_edges=False)
    experimental_settings.add_argument(
        "--reuse_tri_edges", action="store_true", dest="reuse_tri_edges"
    )
    experimental_settings.add_argument(
        "--tri_attn_n_points_per_sample", type=int, default=8
    )
    experimental_settings.add_argument(
        "--tri_attn_n_nbrs_per_sample", type=int, default=16
    )
    experimental_settings.set_defaults(edge_attn_residual=True)
    experimental_settings.add_argument(
        "--no_edge_attn_residual",
        action="store_false",
        dest="edge_attn_residual",
    )
    experimental_settings.set_defaults(checkpoint_tri_attn=True)
    experimental_settings.add_argument(
        "--no_checkpoint_tri_attn",
        action="store_false",
    ...
    args.pre_transformer_gpu_index = default(
        args.pre_transformer_gpu_index, args.gpu_indices[0]
    )
...
def load_refine_args(
    arg_path, curr_args: Optional[RefineArgs], **override
) -> RefineArgs:
