...
class Zero2Zero(nn.Module):
    ...
    def forward(self, x, edge_info):
        ...
        kernels = rearrange(
            self.kernel(edge_info), "... (o i) -> ... o i", o=self.c_out, i=self.c_in
        )
        # kernel shape should be b x n x N x o x i
        return torch.sum(
            rearrange(x, "... i f-> ... f i") * kernels, dim=-1, keepdim=True
        )
...
class Zero2One(nn.Module):
    ...
    def __init__(
        ...
        self.kernel = PairwiseKernel(
            coord_dim_in, dim_out, dim_message, mid_dim=max(mid_dim, dim_message)
        )
    ...
    def forward(self, feats, coords, edges, self_feats=None):
        ...
        if self.use_self_feats:
            ...
            edge_info = torch.cat(
                (feats.squeeze(-1), self_feats.squeeze(-1), edges), dim=-1
            )
        else:
        ...
        #
        weights = rearrange(
            weights, "... (o m) -> ... o m ()", o=self.dim_out, m=self.coord_dim_in
        )
...
class TFNProx(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.zero2zero = Zero2Zero(
            feat_dim_in, feat_dim_out, edge_dim + edge_add, mid_dim=mid_dim
        )
        self.zero2one = Zero2One(
            feat_dim_in,
            coord_dim_in,
            coord_dim_out,
            edge_dim + edge_add,
            mid_dim=mid_dim,
            use_self_feats=use_self_feats,
        )
        self.one2zero = One2Zero(
            coord_dim_in, feat_dim_out, edge_dim + edge_add, mid_dim=mid_dim
        )
        self.one2one = One2One(
            coord_dim_in, coord_dim_out, edge_dim + edge_add, mid_dim=mid_dim
        )
        self.use_self_feats = use_self_feats
    ...
    def forward(self, features, edge_info, rel_dist, basis):
        ...
        self_feats = None
        if self.use_self_feats:
            self_feats = rearrange(feats.squeeze(-1), "b n m -> b n () m").expand_as(
                pairwise_feats
            )
            self_feats = self_feats.unsqueeze(-1)
        pairwise_feats = pairwise_feats.unsqueeze(-1)
        rel_coords = rearrange(coords, "b n m d -> b n () m d") - rearrange(
            coords, "b n m d -> b () n m d"
        )
        ...
        zero2one = self.zero2one(
            pairwise_feats, rel_coords, edges, self_feats=self_feats
        )
