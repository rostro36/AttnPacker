...
from protein_learning.networks.common.net_utils import ReZero as Residual
...
class NodeAttention(nn.Module):  # noqa
    ...
    def __init__(
        ...
        **kawrgs,  # noqa
        ...
        self.bias_softmax, self.sigmoid_gate = (
            Fuser().bias_softmax,
            Fuser().sigmoid_gate,
        )
    ...
    def forward(
        ...
        attn_feats = rearrange(
            self.sigmoid_gate(attn_feats, gate=g), "b h n d -> b n (h d)", h=h
        )
...
class IPA(nn.Module):  # noqa
    ...
    def __init__(
        ...
        **kwargs,  # noqa
        ...
        self.dim_out = heads * (
            default(pair_dim, 0) + dim_value + (4 * dim_value_point)
        )
    ...
    def forward(
        ...
        with disable_tf32(), torch.autocast(device_type=device_type, enabled=False):
            ...
            results_pairwise = (
                einsum("b h i j, b i j d -> b h i d", attn, pair_feats)
                if exists(pair_feats)
                else None
            )
            ...
            results_points = rigids.apply_inverse(
                rearrange(results_points, "b h i d c -> b i (h d) c")
            )
    ...
    def mask_attn_logits(self, mask: Optional[Tensor], *logits):  # noqa
        ...
        mask = rearrange(mask, "b i j -> b () i j")
        fill = (
            lambda x: x.masked_fill(~mask, get_min_val(x)) if torch.is_tensor(x) else x
        )
...
class NodeUpdateBlock(nn.Module):  # noqa
    ...
    def __init__(
        ...
        **net_kwargs,
        ...
        self.attn = kls(
            node_dim=node_dim, pair_dim=pair_dim, dim_out=dim_out, **net_kwargs
        )
        self.attn_residual = Residual()
        self.transition = Transition(
            dim_in=node_dim, mult=default(ff_mult, 2 if use_ipa else 4), dim_out=dim_out
        )
        self.transition_residual = Residual()
        self.attn_dropout = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
        self.transn_dropout = (
            nn.Dropout(dropout) if (dropout > 0 and use_ipa) else nn.Identity()
        )
