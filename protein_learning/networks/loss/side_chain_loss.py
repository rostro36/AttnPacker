...
from protein_learning.common.helpers import exists, masked_mean
...
class SideChainDihedralLoss(nn.Module):
    ...
    def __init__(self, *args, **kwargs):  # noqa
        ...
        self.register_buffer(
            "chi_angle_mask", stack_list(pc.CHI_ANGLES_MASK_LIST).bool()
        )
    ...
    def forward(
        ...
        unnormalized_angles: TensorType[
            "batch", "seq", "no_angles", 2
        ],  # un-normalized predicted angles
        native_coords: TensorType["batch", "seq", 37, 3],
        atom_mask: TensorType["batch", "seq", 37, torch.bool],
    ):
        assert (
            torch.max(sequence) < self.chi_angle_mask.shape[0]
        ), f"{torch.max(sequence)},{self.chi_angle_mask.shape}"
        # b,n,7,2 and b,n,7
        ...
        a_alt_gt = a_gt.clone()
        assert (
            chi_pi_periodic_mask.shape == a_mask.shape
        ), f"{chi_pi_periodic_mask.shape},{a_mask.shape}"
        assert (
            chi_pi_periodic_mask.shape == a_gt.shape[:3]
        ), f"{chi_pi_periodic_mask.shape},{a_gt.shape}"
        a_alt_gt[chi_pi_periodic_mask] = a_alt_gt[chi_pi_periodic_mask] * -1
        a_alt_gt, a_gt = map(
            lambda x: x / (torch.norm(x, dim=-1, keepdim=True) + 1e-8), (a_alt_gt, a_gt)
        )

        not_nan_mask = ~torch.any(torch.isnan(a_gt), dim=-1)
        a_mask = self.chi_angle_mask[sequence] & a_mask.bool() & not_nan_mask
        return self._forward(
            a=unnormalized_angles[..., -4:, :],
            a_gt=a_gt,
            a_alt_gt=a_alt_gt,
            mask=a_mask,
        )
    ...
    def _forward(
        ...
        safe_norm = lambda x, kd=False: torch.sqrt(
            1e-8 + torch.sum(torch.square(x), keepdim=kd, dim=-1)
        )
...
class SideChainDeviationLoss(nn.Module):
    ...
    def forward(
        ...
        if self.p == 2:
            per_res_mse = masked_mean(
                torch.sum(deviations, dim=-1), sc_atom_mask, dim=-1
            )
            return torch.mean(torch.sqrt(per_res_mse[residue_mask] + 1e-8))
        elif self.p == 1:
            deviations = torch.sum(torch.sqrt(deviations + 1e-8), dim=-1)
            return torch.mean(
                masked_mean(deviations, sc_atom_mask, dim=-1)[residue_mask]
            )
        else:
