...
from protein_learning.networks.common.equivariant.linear import LinearInit, VNLinear
...
class FiberResidual(nn.Module):
    ...
    def __init__(self, safe_mode=False, use_re_zero=True):
        ...
        self.alpha = (
            nn.Parameter(torch.zeros(2, requires_grad=True).float() + REZERO_INIT)
            if use_re_zero
            else [1, 1]
        )
    ...
    def __init__(
        ...
        for (degree, dim_in, dim_out) in fiber_in & fiber_out:
            self.projs[str(degree)] = VNLinear(
                dim_in=dim_in, dim_out=dim_out, init=init
            )
...
class FiberFeedForwardResidualBlock(nn.Module):
    def __init__(
        self, feedforward: FiberFeedForward, pre_norm: nn.Module, use_re_zero=True
    ):
        super().__init__()
        self.ff = feedforward
...
class FiberDropout(nn.Module):
    def __init__(self, fiber: Fiber, p=0):
        ...
        self.dropout = nn.ModuleDict(
            {str(d): nn.Dropout(p=p) if p > 0 else nn.Identity() for d in fiber.degrees}
        )
