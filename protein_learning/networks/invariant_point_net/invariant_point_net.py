...
from protein_learning.networks.invariant_point_net.geometric_graph_attention import (
...
class PairUpdate(nn.Module):
    ...
    def __init__(
        ...
        self.outer = LearnedOuterProd(
            dim_in=scalar_dim, dim_out=pair_dim, pre_norm=True
        )
        self.outer_residual = Residual()
        # transition
        self.transition = PreNorm(
            pair_dim, FeedForward(pair_dim, mult=ff_mult, num_layers=num_ff_layers)
        )
...
class InvariantPointNet(nn.Module):
    ...
    def __init__(
        ...
        self.to_rigid_update = (
            nn.Sequential(
                nn.LayerNorm(config.scalar_dim_in), nn.Linear(config.scalar_dim_in, 6)
            )
            if c.use_rigids
            else None
        )
        ...
        for idx in range(config.depth):
            ...
            if config.update_edges:
                ...
                pair_feats = pair_update(
                    scalar_feats=scalar_feats, pair_feats=pair_feats, mask=mask
                )
            ...
            if config.use_rigids:
                ...
                quaternion_update, translation_update = self.to_rigid_update(
                    scalar_feats
                ).chunk(2, dim=-1)
