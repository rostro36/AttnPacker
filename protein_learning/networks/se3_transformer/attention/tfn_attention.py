...
from protein_learning.networks.se3_transformer.attention.se3_attention import (
    SE3Attention,
)
...
class TFNAttention(SE3Attention):
    def __init__(
        ...
        qk_hidden = (
            self.hidden_fiber.scale(2) if linear_project_keys else self.hidden_fiber
        )
        kv_hidden = (
            self.hidden_fiber.scale(2) if not linear_project_keys else self.hidden_fiber
        )
    ...
    def get_qkv(
        ...
        kv, q = self.to_kv(
            features=features, edge_info=edge_info, basis=basis
        ), self.to_q(features)
