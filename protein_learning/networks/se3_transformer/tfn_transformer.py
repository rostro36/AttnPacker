...
class TFNTransformer(SE3Transformer):  # noqa
    ...
    def __init__(
        ...
        super().__init__(
            config=config, freeze=freeze, include_initial_sc=config.fiber_in[1] == 35
        )

        # pre and post TFN layers for mapping between feature types
        conv_config = config.tfn_config(self_interaction=True, pool=True)
        conv_in_config = lambda fiber_in: conv_config.override(
            fiber_in=fiber_in, fiber_out=config.fiber_hidden
        )
        conv_out_config = lambda fiber_out: conv_config.override(
            fiber_in=config.fiber_hidden, fiber_out=fiber_out
        )
        ...
        if config.conv_out_layers > 0:
            ...
            self.conv_out = ConvSE3(
                conv_out_config(
                    fiber_out=config.fiber_hidden
                    if config.project_out
                    else config.fiber_out
                )
            )
    ...
    def project_out(
        ...
        if exists(self.conv_out):
            x = self.conv_out(
                self.conv_out_pre_norm(x), edge_info=edge_info, basis=basis
            )
