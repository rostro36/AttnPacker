...
to_rel_pos = lambda x: rearrange(x, "a b c -> () a b c") - rearrange(
    x, "a b c -> a () b c"
)
...
def kabsch_align(
    ...
    with torch.no_grad():
        rot_to, mean_to, mean_from = _calc_kabsch_rot_n_trans(
            align_to=align_to, align_from=align_from, mask=mask, device=device
        )
...
def _calc_kabsch_rot_n_trans(
    ...
    mean_to, mean_from = map(
        lambda x: masked_mean(x, mask, dim=1, keepdim=True), (align_to, align_from)
    )
    align_to, align_from = align_to - mean_to, align_from - mean_from
    if exists(mask):
        align_to, align_from = map(
            lambda x: torch.masked_fill(x, ~mask, 0), (align_to, align_from)
        )
