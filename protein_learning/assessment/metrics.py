...
from protein_learning.common.protein_constants import AA_INDEX_MAP, ALL_ATOM_POSNS
...
def batch_coords(predicted_coords: Tensor, actual_coords: Tensor, batched_len: int):
    ...
    actual = (
        actual_coords
        if actual_coords.ndim == batched_len
        else actual_coords.unsqueeze(0)
    )
    pred = (
        predicted_coords
        if predicted_coords.ndim == batched_len
        else predicted_coords.unsqueeze(0)
    )
    assert (
        actual.ndim == pred.ndim == batched_len
    ), f"{actual.shape}, {pred.shape}, {batched_len}"
...
def compute_coord_lddt(
    ...
    batched, actual_coords, predicted_coords = batch_coords(
        predicted_coords=predicted_coords, actual_coords=actual_coords, batched_len=3
    )
    ...
    if not exists(pred_rigids):
        ...
        rel_coords = pred_rigids.apply_inverse(
            rearrange(predicted_coords, "b n c -> b () n c")
        )
...
def compute_interface_tm(
    ...
    tm_scale = lambda n: 0.5 if n <= 15 else 1.24 * ((n - 15.0) ** (1.0 / 3.0)) - 1.8
    ...
    _, pair_masks, inter_pair_mask = get_chain_masks(
        n_res=actual_coords.shape[0], chain_indices=chain_indices
    )

    predicted_coords, actual_coords = map(
        lambda x: rearrange(x, "b n c -> b () n c"), (predicted_coords, actual_coords)
    )
...
def get_inter_chain_contacts(
    ...
    part_mask = get_partition_mask(
        n_res=sum([len(p) for p in partition]), partition=partition
    )
...
def compute_interface_rmsd(
    ...
    contacts = get_inter_chain_contacts(
        actual_coords, chain_indices, atom_mask=atom_mask, contact_thresh=contact_thresh
    )
...
def compute_coord_tm(
    ...
    if align:
        _, actual_coords = kabsch_align(
            align_to=predicted_coords, align_from=actual_coords
        )
...
def mean_aligned_error(
    ...
    if align:
        _, actual_coords = kabsch_align(
            align_to=predicted_coords, align_from=actual_coords, mask=mask
        )
...
def per_residue_neighbor_counts(
    atom_coords: Tensor, mask: Optional[Tensor] = None, dist_cutoff=10
):
    ...
    dist_mask = (
        torch.einsum("... n, ... m-> ... nm", mask, mask)
        if exists(mask)
        else torch.ones(1, device=atom_coords.device).bool()
    )
    exclude_self_mask = torch.eye(atom_coords.shape[-2], device=atom_coords.device)
    mask = torch.logical_and(
        dist_mask, exclude_self_mask.unsqueeze(0) if batched else exclude_self_mask
    )
...
def detect_disulfide_bond_pairs(
    target_seq: Tensor, target_coords: Tensor
) -> List[Tuple[int, int]]:
    ...
    is_bond = (
        lambda p1, p2: torch.norm(target_coords[p1, SG] - target_coords[p2, SG]) < 2.5
    )
    return list(
        filter(lambda x: is_bond(*x), combinations(tensor_to_list(cys_posns), 2))
    )
...
def calculate_average_entropy(pred_aa_logits: Tensor):
    ...
    return torch.mean(
        torch.sum(-probs * (log_probs * math.log2(math.e)), dim=-1)
    )  # entropy
...
def calculate_unnormalized_confusion(pred_labels: Tensor, true_labels: Tensor):
    ...
    pred_one_hot, target_one_hot = map(
        lambda x: torch.nn.functional.one_hot(x, 21), (pred_labels, true_labels)
    )
...
def calculate_average_l1_dist_difference(
    predicted_coords: Tensor, actual_coords: Tensor, min_sep: int = 0, max_sep: int = -1
) -> Tensor:
    ...
    sep_mask = get_sep_mask(
        a.shape[1], min_sep=min_sep, max_sep=max_sep, device=a.device
    )
