...
from protein_learning.common.helpers import default, masked_mean, safe_normalize
...
def gather_dihedral_info(
    ...
    dihedral_pred, dihedral_target = map(
        lambda x: safe_normalize(x), (dihedral_pred, dihedral_target)
    )

    # account for pi-peridicity of residues such as ASP, GLU, PHE, TYR
    periodic_mask = torch.tensor(pc.CHI_PI_PERIODIC_LIST, device=sequence.device)[
        sequence
    ].bool()
    ...
    diff, alt_diff = angle_diff(dihedral_pred, dihedral_target), angle_diff(
        dihedral_pred, alt_target
    )
...
def gather_dihedral_info_of(
    ...
    dihedral_pred, dihedral_target = map(
        dihedral_info, (predicted_coords, target_coords)
    )

    # account for pi-peridicity of residues such as ASP, GLU, PHE, TYR
    angle_diff = lambda key: torch.sum(
        torch.square(dihedral_pred["torsion_angles_sin_cos"] - dihedral_target[key]),
        dim=-1,
    )
    diff, alt_diff = angle_diff("torsion_angles_sin_cos"), angle_diff(
        "alt_torsion_angles_sin_cos"
    )
    # swap for the unit-vec that is closer to the predicted for these residues
    dihedral_target["torsion_angles_sin_cos"][diff > alt_diff] = dihedral_target[
        "alt_torsion_angles_sin_cos"
    ][diff > alt_diff]
...
def gather_target_stats(
    ...
    output["predicted_coords"] = predicted_coords
    output["rmsd"]["per-prot"] = compute_sc_rmsd(coords=predicted_coords,target_coords=target_coords,atom_mask=atom_mask,sequence=sequence,per_residue=False)
    # number of CB neighbors in 10A ball
    atom_mask_size = atom_mask.size()
    centrality = compute_degree_centrality(target_coords).unsqueeze(2)
    centrality = centrality.repeat(1,1,atom_mask_size[2])
    central = atom_mask&(centrality>19)
    output["central"] = central
    surface = atom_mask&(centrality<16)
    output["rmsd"]["per-res-central"] = compute_sc_rmsd(
        coords=predicted_coords,
        target_coords=target_coords,
        atom_mask=central,
        sequence=sequence,
    ...
    output["rmsd"]["per-prot-central"] = compute_sc_rmsd(coords=predicted_coords,target_coords=target_coords,atom_mask=central,sequence=sequence,per_residue=False)
    
    output["rmsd"]["per-res-surface"] = compute_sc_rmsd(
        coords=predicted_coords,
        target_coords=target_coords,
        atom_mask=surface,
        sequence=sequence,
    ...
    output["rmsd"]["per-res-mask-surface"] = torch.any(atom_mask[..., 4:], dim=-1)
    output["rmsd"]["per-prot-surface"] = compute_sc_rmsd(coords=predicted_coords,target_coords=target_coords,atom_mask=surface,sequence=sequence,per_residue=False)
    # steric clash info
    output["clash-info"] = {
        str(round(100 * tol_frac)): compute_clashes(
            predicted_coords,
            atom_mask=atom_mask,
            sequence=sequence,
    ...
    dihedral_fn = (
        gather_dihedral_info_of if use_openfold_dihedral_calc else gather_dihedral_info
    )
    ...
    ca_native, ca_target = map(
        lambda x: x[..., 1, :].clone(), (target_coords, predicted_coords)
    )
    # need to add small epsilon here so that matrix for SVD is non-singular
    ca_native, ca_target = kabsch_align(
        align_to=ca_target, align_from=ca_native + torch.randn_like(ca_native) * 1e-7
    )
    output["ca_rmsd"] = torch.sqrt(
        torch.mean(torch.sum(torch.square(ca_native - ca_target), dim=-1))
    )
    return output
...
def assess_sidechains(
    ...
    return map_tensor_fn_onto_iterable(
        stats, lambda x: x.detach().cpu().squeeze(0) if torch.is_tensor(x) else x
    )
