...
from protein_learning.features.feature_config import FeatureEmbeddingTy, FeatureTy
...
class Feature:
    ...
    def __init__(
        ...
        encoding_ty: Optional[
            Union[List[FeatureEmbeddingTy], FeatureEmbeddingTy]
        ] = None,
        **kwargs,
    ):
        """
    ...
    def get_raw_data(self):
        ...
        if self.masked:
            if not exists(self.raw_mask_value):
                raise Exception(
                    f"feature {self.name} has been masked,"
    ...
    def device(self):
        ...
        return (
            self.encoded_data.device
            if exists(self.encoded_data)
            else self.raw_data.device
        )
