...
class GraphTransformer(nn.Module):  # noqa
    ...
    def __init__(self, config: GeomGTConfig):
        ...
        self.to_rigid_update = (
            nn.Sequential(nn.LayerNorm(c.node_dim), nn.Linear(c.node_dim, 6))
            if c.use_ipa
            else None
        )
        ...
        self.pair_updates = (
            ...
            if exists(c.pair_update_kwargs)
            else [None] * depth
        )

    def update_rigids(
        self, feats: Tensor, rigids: Rigids, msk: Optional[Tensor] = None
    ) -> Optional[Rigids]:
        ...
        if exists(self.to_rigid_update):
            ...
            quaternion_update = F.pad(quaternion_update, (1, 0), value=1.0)  # noqa
    ...
    def forward(
        ...
        assert (
            node_feats.ndim == 3 and pair_feats.ndim == 4
        ), f"scalar and pair feats must have batch dimension!"
        ...
        if self.use_ipa:
            ...
            rigids = (
                rigids
                if exists(rigids)
                else Rigids.IdentityRigid(leading_shape=(b, n), device=device)
            )  # noqa
        ...
        if exists(rigid_update_mask) and exists(true_rigids) and self.use_ipa:
            ...
            rigids.translations[0, ~msk] = centroid * torch.ones_like(
                rigids.translations[0, ~msk]
            )
            trans, quats = (
                true_rigids.translations[0, msk],
                true_rigids.quaternions[0, msk],
            )
        ...
        node_updates, pair_updates, aux_loss = (
            self.node_updates,
            self.pair_updates,
            None,
        )
        ...
        forward_kwargs = dict(
            node_feats=node_feats, pair_feats=pair_feats, mask=pair_mask
        )
    ...
    def aux_loss(
        ...
        assert pred_ca.shape == native_ca.shape, f"{pred_ca.shape, native_ca.shape}"
        pred_ca, native_ca = map(
            lambda x: rearrange(x, "b n c -> b n () c"), (pred_ca, native_ca)
        )
