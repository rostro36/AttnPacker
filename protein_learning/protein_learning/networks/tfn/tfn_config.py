...
class TFNConfig(NamedTuple):
    ...
    def get_edge_features(
        self, edges: Optional[Tensor], nbr_info: NeighborInfo
    ) -> Tensor:
        ...
        if self.fourier_encode_dist:
            fourier_rel_dist = fourier_encode(
                rel_dist, num_encodings=self.num_fourier_features, include_self=False
            )
            edge_features = torch.cat(
                (fourier_rel_dist.detach(), edge_features), dim=-1
            )
