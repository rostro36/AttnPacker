...
from protein_learning.common.data.datasets.utils import get_ab_ag_spatial_crop
...
class AbDataset(ProteinDataset):
    ...
    def get_item_from_pdbs_n_seq(
        ...
        assert isinstance(
            native_pdb_path, list
        ), f"[ERROR] dataloader - native pdbs is not a list"
        include_light = True
        try:
            ...
            if heavy is None or not os.path.exists(heavy):
                ...
                if (native_pdb_path[1] is not None) and os.path.exists(
                    native_pdb_path[1]
                ):
            ...
            if self.native_folder != self.decoy_folder:
                ...
                seq = ab_ag_decoy.seq
                seqs = [
                    seq[int(sum(chain_lens[:i])) : int(sum(chain_lens[: i + 1]))]
                    for i in range(3)
                ]
            ...
            if self.crop_len > 0:
                if ab_ag.n_chains > 1:
                    ...
                    if self.crop_len < len(ab_ag):
                        crop_posns = get_ab_ag_spatial_crop(
                            ab_ag, crop_len=self.crop_len
                        )
                        ab_ag, ab_ag_decoy = map(
                            lambda x: x.restrict_to(crop_posns), (ab_ag, ab_ag_decoy)
                        )
