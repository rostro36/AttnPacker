...
class FeatureFlagGen:
    ...
    def __init__(
        ...
        self.include_terminal_flags = include_terminal_flags
        self.num_interface = (
            num_interface if 0 < num_interface < 1 else int(num_interface)
        )
    ...
    def gen_flags(
        ...
        if self.include_bond_flags:
            pair_flags.append(
                get_bond_flags(native_protein, all_bonded=self.include_all_bonded)
            )
        ...
        if self.include_interface_flags:
            if not exists(self.contact_n_interface_dict):
                ...
                i_flags = get_interface_flags_from_dict(
                    decoy_protein, data=self.contact_n_interface_dict, index=curr_idx
                )
                res_flags.append(i_flags)
        ...
        if self.include_block_adjs:
            ...
            if self.ignore_inter_chain_ss:
                if decoy_protein.is_complex:
                    pmask = get_partition_mask(
                        len(decoy_protein), decoy_protein.chain_indices
                    )
            ...
            _feats = torch.cat((block_adjs, block_oris.unsqueeze(-1)), dim=-1)
        ...
        res_flags, pair_flags = map(
            lambda x: torch.cat(x, dim=-1) if len(x) > 0 else None,
            (res_flags, pair_flags),
        )
    ...
    def _update_contact_dict_state(self, protein: Protein):
        try:
            ...
            print(
                f"{INFO} loaded contact and interface info for"
    ...
    def flag_dims(self):  # noqa
        node_flag_dim = (
            int(self.include_interface_flags)
...
def get_bond_flags(
    protein: Protein, coords: Optional[Tensor] = None, all_bonded=False
) -> Tensor:
...
def _load_dict_entry(protein: Protein, data: Dict, index: int, key: str):
    try:
        ...
        if "sizes" in data[k]:
            ...
            sizes = data[k]["sizes"]
            assert (
                len(c1) == sizes[0] and len(c2) == sizes[1]
            ), f"got chain lens: ({len(c1)},{len(c2)}), expected: {sizes}"
        index = index % len(data[k][key])
        return data[k][key][index]
    except Exception as e:
        print(
            f"{ERROR} {e} loading contact dict enrty for protein {protein_to_name(protein)}"
        )
        raise e
...
def get_contact_flags_from_dict(protein: Protein, data: Dict, index: int) -> Tensor:
    try:
        ...
        print(
            f"loaded {len(contacts)} contacts for target {protein_to_name(protein)} (replica : {index})"
        )
...
def get_interface_flags_from_dict(protein: Protein, data: Dict, index: int) -> Tensor:
    try:
        ...
        for i in interface:
            interface_flags[i] = 1
        print(
            f"loaded {len(interface)} interface residues for target {protein_to_name(protein)} (replica : {index})\n"
            f"interface: {interface}\nchain lengths : {list(map(len, protein.chain_indices))}"
        )
        return interface_flags.unsqueeze(-1)
    except Exception as e:
        print(
            f"{ERROR} {e} loading interface res data for protein {protein_to_name(protein)}"
        )
        raise e
    ...
    if include_contact_prob > 0 and len(chain_indices) >= 2:
        if random.random() <= include_contact_prob:
            ...
            if not exists(num_contact) and not exists(random_contact):
                ...
                n_con = (
                    np.random.geometric(num_contact, 1)[0]
                    if 0 < num_contact < 1
                    else num_contact
                )
                ...
                rc1, rc2 = map(
                    lambda x: np.random.choice(x, size=random_contact, replace=True),
                    (c1, c2),
                )
                rc1, rc2 = map(
                    lambda x: x if isinstance(x, np.ndarray) else [x], (rc1, rc2)
                )
    ...
    if include_prob > 0 and len(chain_indices) >= 2:
        if random.random() < include_prob:
            ...
            if not exists(num_interface):
                ...
                if len(cp1) > 0:
                    n_int = (
                        np.random.geometric(num_interface) - 1
                        if 0 < num_interface < 1
                        else num_interface
                    )
                    cp1 = np.random.choice(
                        cp1, size=min(len(cp1), n_int), replace=False
                    )
                if len(cp2) > 0:
                    n_int = (
                        np.random.geometric(num_interface) - 1
                    ...
                    cp2 = np.random.choice(
                        cp2, size=min(len(cp2), n_int), replace=False
                    )
            ...
            if exists(random_interface) and random_interface > 0:
                ...
                pocket_res[part] = 0
                rand_pocket = np.random.choice(
                    part.numpy(), size=random_interface, replace=False
                )
                rand_pocket = (
                    rand_pocket
                    if isinstance(rand_pocket, np.ndarray)
                    else [rand_pocket]
                )
...
def get_interface_scores(
    partition: List[Tensor], coords: Tensor, sigma: float = 3
) -> Tensor:
    """Assign score to each residue based on proximity to interface"""
    assert coords.ndim == 2, f"{coords.ndim}"
    assert len(coords) == sum(
        [len(x) for x in partition]
    ...
    for i, idxs_i in enumerate(ss_blocks):
        for j, idxs_j in enumerate(ss_blocks):
            ...
            if dist < thresh:
                ...
                u, v = map(
                    lambda idxs: ca_coords[idxs[-1]] - ca_coords[idxs[0]],
                    (idxs_i, idxs_j),
                )
