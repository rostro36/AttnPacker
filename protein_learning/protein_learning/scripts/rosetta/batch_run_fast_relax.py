...
def add_ca_dist_cons(pose, sfxn, wt=0, d_max=1e6):
    if wt > 0:
        ...
        print(dists.shape)
        func = lambda i, j, sigma=1: prs.rosetta.core.scoring.func.HarmonicFunc(
            dists[i - 1, j - 1], 1
        )
        constraint = (
            lambda i, j: prs.rosetta.core.scoring.constraints.AtomPairConstraint(
                atom_id(i), atom_id(j), func(i, j)
            )
        )
    ...
    for pdb in (pdb1, pdb2):
        with open(pdb, "r+") as pin:
            for x in pin:
                if not any(
                    [
                        x.strip().startswith(y)
                        for y in "REMARK END TER HEADER MODEL".split()
                    ]
                ):
                    lines.append(x)
        lines.append("TER\n")
...
def run_relax(
    ...
    for decoy in range(n_decoys):
        ...
        print(
            f"{pdb_name} RMSD : {prs.rosetta.core.scoring.CA_rmsd(original_pose, test_pose)}"
        )
...
to_pdb_file = lambda x, fldr=args.pdb_folder: os.path.join(
    fldr, x + ("" if x.endswith(".pdb") else ".pdb")
)
...
if args.is_ab_ag:
    H_chains, L_chains, A_chains = map(
        lambda i: [x.strip().split()[i] for x in model_list], range(3)
    )
