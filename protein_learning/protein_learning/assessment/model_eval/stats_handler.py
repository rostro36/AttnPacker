...
from protein_learning.common.protein_constants import AA_TO_INDEX, INDEX_TO_AA_ONE
...
class LossHandler:
    ...
    def get_interface_mask(
        model_out: ModelOutput, contact_thresh: float, mask: Optional[Tensor]
    ) -> Tensor:
    ...
    def get_contact_data(self, model_out: ModelOutput):
        ...
        """
        out = {}
        pred_ca, native_ca, ca_mask = model_out.get_pred_and_native_coords_and_mask(
            atom_tys=["CA"]
        )
        pred_ca, native_ca, ca_mask = map(
            lambda x: x.squeeze(), (pred_ca, native_ca, ca_mask)
        )
        chain_indices = model_out.decoy_protein.chain_indices
        ...
        for c_idx, indices in enumerate(chain_indices):
            for sep in [(5, 1e5), (11, 1e5), (23, 1e5)]:
                if len(indices) > (sep[0] * 2):
                    ...
                    out["chain-contact-precision"][c_idx][
                        sep[0] + 1
                    ] = get_contact_precision(
        ...
        if len(chain_indices) > 1:
            ...
            for threshold in [6, 8, 12]:
                interface_mask = self.get_interface_mask(
                    model_out, contact_thresh=threshold, mask=ca_mask
                )
                ...
                n_contacts = torch.sum(
                    out["true_interface_contacts"][threshold].float()
                )
                out["n_contacts"][threshold] = n_contacts
                out["interface-contact-precision"][threshold] = (
                    get_contact_precision(
                        pred_coords=pred_ca,
                ...
                out["interface-contact-recall"][threshold] = (
                    get_contact_recall(
    ...
    def _get_dist_data(model_out, loss_fn):
        ...
        return dict(
            ...
            probs={aty: probs[..., i, :] for i, aty in enumerate(loss_fn.atom_tys[:1])},
            masks={aty: masks[..., i] for i, aty in enumerate(loss_fn.atom_tys[:1])},
            bins=bins,
        )
    ...
    def get_lddt_data(self, model_out: ModelOutput) -> Dict:
        ...
        if exists(self.loss_fn) and LossTy.PLDDT in self.loss_fn:
            ...
            out["plddt"], out["plddt-mask"] = plddt_net.forward_from_output(
                model_out, return_true_prop=True
            )[1:]
    ...
    def get_nsr_data(self, model_out: ModelOutput) -> Dict:
        ...
        logits = loss_fn.get_predicted_logits(
            residue_feats=model_out.scalar_output
        ).squeeze()
        pred_labels = torch.argmax(logits, dim=-1)
        pred_seq = "".join(
            [
                INDEX_TO_AA_ONE[label.item()]
                if label.item() in INDEX_TO_AA_ONE
                else "-"
        ...
        true_labels = torch.tensor(
            [
                AA_TO_INDEX[s] if s in AA_TO_INDEX else 20
                for i, s in enumerate(native_seq)
            ],
            device=logits.device,
        ).long()

        correct_preds = pred_labels == true_labels
        acc = correct_preds[correct_preds].numel() / max(
            1, correct_preds.numel()
        )  # noqa
        seq_mask = model_out.model_input.input_features.seq_mask.to(
            correct_preds.device
        )  # noqa
        correct_preds = correct_preds[seq_mask]  # noqa
        if correct_preds.numel() > 0:
            masked_acc = correct_preds[correct_preds].numel() / max(
                1, correct_preds.numel()
            )  # noqa
        else:
            masked_acc = 0
    ...
    def get_rmsd_data(self, model_out: ModelOutput) -> Dict:
        ...
        masked_pred_ca, masked_native_ca, masked_decoy_ca = map(
            lambda x: x[ca_mask], (pred_ca, native_ca, decoy_ca)
        )
    ...
    def get_ab_stats(self, model_out: ModelOutput):
        ...
        for mask, name in ((res_mask, "framework"), (full_mask, "cdr")):
            pred_coords = (
                model_out.predicted_coords[..., 1, :].squeeze()[ab_chain].unsqueeze(0)
            )
            native_coords = model_out.native["CA"].squeeze()[ab_chain].unsqueeze(0)
            aln_pred, aln_native = kabsch_align(
                align_to=pred_coords, align_from=native_coords, mask=mask.unsqueeze(0)
            )
