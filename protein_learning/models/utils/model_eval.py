    ...
    def evaluate_dataset(self, dataset: ChainDataset, n_cycles: int = 1, **kwargs):
        ...
        for _ in range(n_cycles):
            data = dataset.get_dataloader(
                batch_size=min(len(dataset), 32), num_workers=4, shuffle=False
            )
            n_processed = 0
                    ...
                    for sample in batch_data:
                        try:
                            if self.use_custom_forward:
                                model_out = self.custom_forward(
                                    trainer.model, sample, **kwargs
                                )
                            else:
                                model_out = trainer.safe_eval(
                                    sample,
                            ...
                            if n_processed % max(1, (len(dataset) // 10)) == 0:
                                print(
                                    f"[INFO] progress {1000 * (n_processed / len(dataset)) // 10}%, "
